...
static void focusstackafterswitch(const Arg *arg);
...
void focus(Client *c) {
  if (!c || !ISVISIBLE(c, selmon) || HIDDEN(c))
    for (c = selmon->cl->stack; c && (!ISVISIBLE(c, selmon) || HIDDEN(c));
         c = c->snext)
      ;
...
void focusstackafterswitch(const Arg *arg) {
  Client *c = NULL;

  if (!selmon->sel)
    return;
  /*this is the different loop comparing to the focusstack method*/
  /*change the for loop to stop at the last active window after switch*/
  for (c = selmon->sel; c && !ISVISIBLE(c, selmon); c = c->next)
    ;
  if (!c)
    for (c = selmon->cl->clients; c && !ISVISIBLE(c, selmon); c = c->next)
      ;
...
void keyhiddenwin(const Arg *arg) {
  if (!selmon->sel)
    return;
...
void keyrestorewin(const Arg *arg) {
